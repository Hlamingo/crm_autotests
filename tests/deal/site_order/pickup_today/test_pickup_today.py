...
params = ["test_case_1.json", "test_case_2.json", "test_case_3.json", "test_case_4.json"]
...
class TestSiteOrder(DealBaseTest):
    ...
    def test_check_assigned_for_deal(self):
        assigned_for_deal = self.deal_details_page.assigned_block()
        assigned_id = assigned_for_deal[0].get_attribute("href").split("/")[-2]
        assert assigned_id == self.assigned_by_id
    ...
    @allure.title("Проверка стадии сделка на основании остатков товара")
    ...
    def test_reserve_interface_page(self, temp_file):
        if temp_file.name == "test_case_4.json":
            pytest.skip("Для кейса 'Все товары отсутствуют' ИР недоступен")
        data = read_file(temp_file)
        deal_id = data["result"]["ID"]
